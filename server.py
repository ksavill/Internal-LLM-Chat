        ...
        try:
            if chat_req.stream:
                ...
                async def streamer() -> AsyncGenerator[bytes, None]:
                    ...
                    if chat_req.tools:
                        # Forward the first chunk in its raw form.
                        yield _json_bytes(first_chunk)  # type: ignore[arg-type]

                        # Attempt to extract any assistant text so we can
                        # persist it later.  This is best-effort and safe to
                        # ignore on failure.
                        try:
                            first_content_candidate = selected_interface.extract_content_from_chunk(  # type: ignore
                                first_chunk if isinstance(first_chunk, dict) else {}
                            )
                            if first_content_candidate:
                                full_response_chunks.append(first_content_candidate)
                        except Exception as _exc:
                            logger.debug(
                                "Unable to extract content from first tool chunk: %s", _exc
                            )
                    else:
                    ...
                    try:
                        async for chunk in selected_stream_gen:  # type: ignore
                            ...
                            if chat_req.tools:
                                # Forward full chunk to client **and** attempt
                                # to accumulate any plain-text assistant content
                                # so that it can be stored in the database once
                                # the stream finishes.  This mirrors the logic
                                # used when *tools* are not involved but keeps
                                # the original full-fidelity payload for the
                                # client.

                                yield _json_bytes(chunk)

                                try:
                                    content_candidate = selected_interface.extract_content_from_chunk(  # type: ignore
                                        chunk_dict
                                    )
                                    if content_candidate:
                                        full_response_chunks.append(content_candidate)
                                except Exception as _exc:
                                    logger.debug(
                                        "Failed to extract content from tool chunk: %s", _exc
                                    )
                            else:
                ...
                async def save_streamed_response_task():
                    ...
                    # When *tools* are supplied we purposefully do **not**
                    # extract/concatenate partial assistant chunks above
                    # because the structure can vary (tool calls, deltas,
                    # etc.).  That means *full_response_chunks* will remain
                    # empty even though the request is valid.  Previously we
                    # aborted the DB update in that situation which caused
                    # the **user's** latest message (and any tool results)
                    # to be lost from the persistent conversation history –
                    # leading to the “perpetual amnesia” behaviour after a
                    # page refresh.

                    final_content = "".join(full_response_chunks)

                    # We still want to record the conversation even when the
                    # assistant hasn't produced plain-text content (e.g. the
                    # assistant only returned a tool call).  Therefore we
                    # keep *final_content* as-is but continue with the DB
                    # update regardless of whether it is empty.
                    ...
                    def _save_in_thread():
                        ...
                        # If the assistant emitted text we store it as usual.
                        # Otherwise we simply persist the conversation up to
                        # the last *user* message so that history is not
                        # lost.  Tool-call payloads are not stored yet – that
                        # would require a richer schema and is outside the
                        # scope of this fix.

                        if final_content.strip():
                            new_assistant_msg_pydantic = ChatMessage(
                                role="assistant",
                                content=final_content,
                                images=None,
                            )
                            all_messages_for_db_pydantic = (
                                combined_for_model_pydantic
                                + [new_assistant_msg_pydantic]
                            )
                        else:
                            all_messages_for_db_pydantic = combined_for_model_pydantic
                ...
                if conversation_id:
                    # Persist the conversation history even when the request
                    # involves **tools** so that the next turn contains the
                    # full context.  The helper above already copes with the
                    # absence of assistant text content when only tool calls
                    # were produced.
